...
class DatabaseService {
  ...
  constructor(db) {
    ...
    this._db.exec(`CREATE TABLE users(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password TEXT NOT NULL) STRICT`);

    // populate with default data.

    const insert = this._db.prepare('INSERT INTO users (username, password) VALUES (?, ?)');
    users.forEach((user) => {
    ...
    this._db.exec(`CREATE TABLE refresh_tokens (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, token TEXT NOT NULL UNIQUE) STRICT`);
  }
  ...
  async addRefreshToken(token) {
    try {
      async function insertToken(db, token) {
        db.run(`INSERT INTO refresh_tokens (token) VALUES (?)`, [token], function (err) {
          if (err) {
